...
import { handleIssueSubmission } from '@/app/lodge-issue/actions';
...
export default function IssueForm() {
  ...
  async function uploadFilesToFirebase(issueId: string, files: File[]): Promise<string[]> {
    if (!storage) {
  ...
  async function onSubmit(values: FormValues) {
    ...
    try {
      // Step 1: Create issue document to get an ID (without image URLs)
      const initialIssueData = { ...values, imageUrls: [], imageDataUris: [] };
      const initialResult = await handleIssueSubmission(initialIssueData);

      if (!initialResult.success || !initialResult.issueId) {
        throw new Error(initialResult.error || 'Failed to create the initial issue document.');
      }
      
      const issueId = initialResult.issueId;

      // Step 2: Upload files to Firebase Storage using the new issue ID
      const filesToUpload = values.attachments || [];
      const imageUrls = await uploadFilesToFirebase(issueId, filesToUpload);
      ...
      if (firestore) {
          ...
          await updateDoc(issueRef, { imageUrls });
          
          // Re-trigger the server action, this time with all data
          await handleIssueSubmission({ ...values, imageUrls, imageDataUris, issueId });
      } else {
        throw new Error("Firestore not available to finalize issue.");
      }
  ...
  const handleCopyToClipboard = () => {
    if (submittedIssueId) {
      ...
      toast({
        ...
        description: 'The issue ID has been copied.',
      });
    }
  };
      ...
      <AlertDialog open={showSuccessDialog} onOpenChange={setShowSuccessDialog}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogTitle className="text-center">Issue Submitted!</AlertDialogTitle>
            <AlertDialogDescription className="text-center">
              Your issue has been successfully submitted. Please save your Tracking PIN.
            </AlertDialogDescription>
